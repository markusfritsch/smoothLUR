...
# Focus on traffic/industrial sites only
df.TrInd <- df.NO2.meta[df.NO2.meta$AirQualityStationType!="background",]

unique(droplevels(df.NO2.meta$DatetimeBegin))
unique(droplevels(df.NO2.meta$DatetimeEnd))

# Focus on one year only
df.TrInd <- df.TrInd[df.TrInd$DatetimeBegin=="2015-01-01 00:00:00",]


# Identify duplicates
df.TrInd$AirQualityStationEoICode[duplicated(df.TrInd$AirQualityStationEoICode)]

summary(df.TrInd$MeasurementType)
#  active automatic   passive
#       0       157        14
#
# Remove duplicates
df.TrInd <- df.TrInd[df.TrInd$MeasurementType=="automatic",]
...
df.TrInd <- cbind(df.TrInd,data.frame(matrix(data = 0, nrow = nrow(df.TrInd), ncol = 10)))
names(df.TrInd)[37:46] <- paste(rep("Class", n = 10), seq(from = 1, to = 10, by = 1), sep = "")
...
unique(df.TrInd$Projection)
# [1] EPSG:4979
# +proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs

sp.TrInd <- SpatialPoints(coords = df.TrInd[,c("Longitude","Latitude")],
                               proj4string = CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs"))
sp.TrInd <- spTransform(sp.TrInd,proj4string(raster.CLC12))

radius <- 1000

for(i in 1:nrow(df.TrInd)){
  list  <- extract(raster.CLC12,
                   coordinates(sp.TrInd[i,]),
                   buffer=radius)
  
  nr.cells <- length(list[[1]])
  
  df.TrInd$Class1[i] <- sum(list[[1]]==1)/nr.cells
  df.TrInd$Class2[i] <- sum(list[[1]]==2)/nr.cells
  df.TrInd$Class3[i] <- sum(list[[1]]==3)/nr.cells
  df.TrInd$Class4[i] <- sum(list[[1]]==4)/nr.cells
  df.TrInd$Class5[i] <- sum(list[[1]]==5)/nr.cells
  df.TrInd$Class6[i] <- sum(list[[1]]==6)/nr.cells
  df.TrInd$Class7[i] <- sum(list[[1]]%in%c(7:9))/nr.cells
  df.TrInd$Class8[i] <- sum(list[[1]]%in%c(10:11))/nr.cells
  df.TrInd$Class9[i] <- sum(list[[1]]%in%c(12:22))/nr.cells
  df.TrInd$Class10[i] <- sum(list[[1]]%in%c(23:25))/nr.cells
}
...
proj4string(sp.TrInd)


df.TrInd2 <- cbind(df.TrInd,over(spTransform(sp.TrInd, proj4string(regions.2015.complete)),regions.2015.complete))
summary(df.TrInd2$AGS)
df.TrInd2[is.na(df.TrInd2$AGS),]
# Search on the internet for monitoring sie "DEMV031" -> is located in Rostock
# Search on the internet for the AGS of Rostock -> is attributed to "13003000"
...
which(is.na(df.TrInd2$AGS))
# [1] 75
plot(spTransform(sp.TrInd[75,],proj4string(regions.2015.complete)),col="red",pch=16,cex=1,add=TRUE)
# Because it is on the border of the respective polygon...
# Add the information manually
names(df.TrInd2)
names(regions.2015.complete)
df.TrInd2[df.TrInd2$AirQualityStationEoICode=="DEMV031",47:58] <- regions.2015.complete@data[regions.2015.complete$AGS=="13003000",]

summary(duplicated(df.popDens2015$vbgem15)) # unambiguously identified
df.TrInd3 <- merge(df.TrInd2, df.popDens2015[,c(1,3,4)], by="vbgem15",all.x=TRUE)
df.TrInd3$popDens15 <- df.TrInd3$z_bev15/df.TrInd3$fl15_sum
...
sp.TrInd3 <- SpatialPoints(coords = df.TrInd3[,c("Longitude","Latitude")],
                           proj4string = CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs"))
sp.TrInd3 <- spTransform(sp.TrInd3,proj4string(raster.CLC12))


spdf.TrInd3 <- SpatialPointsDataFrame(sp.TrInd3,data=df.TrInd3)
BufferSites <- gBuffer(spdf.TrInd3, byid = TRUE,
                       id = spdf.TrInd3$AirQualityStationEoICode,
                       width = radius)
...
for(i in 1:nrow(df.TrInd3)){
  ...
  if(nrow(sLdf.tmp)>0){
    ...
    if(nrow(sLdf.tmp2.14)>0){
      df.TrInd3$Road14[i] <- sum(SpatialLinesLengths(sLdf.tmp2.14))
    }
    if(nrow(sLdf.tmp2.15)>0){
      df.TrInd3$Road15[i] <- sum(SpatialLinesLengths(sLdf.tmp2.15))
    }
    if(nrow(sLdf.tmp2.16)>0){
      df.TrInd3$Road16[i] <- sum(SpatialLinesLengths(sLdf.tmp2.16))
    }
    if(nrow(sLdf.tmp2.984)>0){
      df.TrInd3$Road984[i] <- sum(SpatialLinesLengths(sLdf.tmp2.984))
    }
  }
}
...
sLdf.roads.DE2 <- spTransform(sLdf.roads.DE, proj4string(spdf.TrInd3))
...
dat.ProximityToRoads <- data.frame(ID = df.TrInd3$AirQualityStationEoICode,
                                   Type = df.TrInd3$AirQualityStationType,
                                   Lon.WGS84 = df.TrInd3$Longitude,
                                   Lat.WGS84 = df.TrInd3$Latitude,
                                   DistPri = rep(NA, nrow(df.TrInd3)),
                                   DistSec = rep(NA, nrow(df.TrInd3)),
                                   DistMot = rep(NA, nrow(df.TrInd3)),
                                   DistLoc = rep(NA, nrow(df.TrInd3)))


for(i in 1:nrow(df.TrInd3)){
  dat.ProximityToRoads$DistPri[i] <- gDistance(spdf.TrInd3[i,], sLdf.roads.DE2.14)
  dat.ProximityToRoads$DistSec[i] <- gDistance(spdf.TrInd3[i,], sLdf.roads.DE2.15)
  dat.ProximityToRoads$DistMot[i] <- gDistance(spdf.TrInd3[i,], sLdf.roads.DE2.16)
  dat.ProximityToRoads$DistLoc[i] <- gDistance(spdf.TrInd3[i,], sLdf.roads.DE2.984)
}


dat.ProximityToRoads[,5:8] <- round(dat.ProximityToRoads[,5:8], 2)

write.csv(dat.ProximityToRoads, file = "R/DATA/Data_built/dat_ProximityToRoads_traffic_industrial.csv")




names(df.TrInd3)
df.analysis <- df.TrInd3[,c(2,11,24,26:29,31:32,35,38:48,50:51,57:58,61:65)]
...
write.csv(df.analysis, file = "R/DATA/Data_built/DATA_DE_traffic_industrial.csv")
