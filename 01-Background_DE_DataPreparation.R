...
df.Background <- df.Background[df.Background$DatetimeBegin=="2015-01-01 00:00:00",]


# Identify duplicates
df.Background$AirQualityStationEoICode[duplicated(df.Background$AirQualityStationEoICode)]
...
summary(df.Background$MeasurementType)
#  active automatic   passive
#       0       246         4
#
# Remove duplicates
df.Background <- df.Background[df.Background$MeasurementType=="automatic",]
...
df.Background <- cbind(df.Background,data.frame(matrix(data = 0, nrow = nrow(df.Background), ncol = 10)))
names(df.Background)[37:46] <- paste(rep("Class", n = 10), seq(from = 1, to = 10, by = 1), sep = "")
...
unique(df.Background$Projection)
# [1] EPSG:4979
# +proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs

sp.Background <- SpatialPoints(coords = df.Background[,c("Longitude","Latitude")],
                              proj4string = CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs"))
sp.Background <- spTransform(sp.Background,proj4string(raster.CLC12))

radius <- 1000

for(i in 1:nrow(df.Background)){
  list  <- extract(raster.CLC12,
                    coordinates(sp.Background[i,]),
                    buffer=radius)

   nr.cells <- length(list[[1]])

   df.Background$Class1[i] <- sum(list[[1]]==1)/nr.cells
   df.Background$Class2[i] <- sum(list[[1]]==2)/nr.cells
   df.Background$Class3[i] <- sum(list[[1]]==3)/nr.cells
   df.Background$Class4[i] <- sum(list[[1]]==4)/nr.cells
   df.Background$Class5[i] <- sum(list[[1]]==5)/nr.cells
   df.Background$Class6[i] <- sum(list[[1]]==6)/nr.cells
   df.Background$Class7[i] <- sum(list[[1]]%in%c(7:9))/nr.cells
   df.Background$Class8[i] <- sum(list[[1]]%in%c(10:11))/nr.cells
   df.Background$Class9[i] <- sum(list[[1]]%in%c(12:22))/nr.cells
   df.Background$Class10[i] <- sum(list[[1]]%in%c(23:25))/nr.cells
 }
...
proj4string(sp.Background)


df.Background2 <- cbind(df.Background,over(spTransform(sp.Background,proj4string(regions.2015.complete)),regions.2015.complete))
summary(df.Background2$AGS)
df.Background2[is.na(df.Background2$AGS),]
# Search on the internet for monitoring sie "DESN024" -> is located in Klingenthal
# Search on the internet for the AGS of Klingenthal -> is attributed to "14523160"
# Why is it not attributed to a polygon in "regions.2015.complete"
plot(regions.2015.complete[regions.2015.complete$AGS=="14523160",])
which(is.na(df.Background2$AGS))
# [1] 203
plot(spTransform(sp.Background[203,],proj4string(regions.2015.complete)),col="red",pch=16,cex=1,add=TRUE)
# Because it is on the border of the respective polygon...
# Add the information manually
names(df.Background2)
names(regions.2015.complete)
df.Background2[df.Background2$AirQualityStationEoICode=="DESN024",47:58] <- regions.2015.complete@data[regions.2015.complete$AGS=="14523160",]

summary(duplicated(df.popDens2015$vbgem15)) # unambiguously identified
df.Background3 <- merge(df.Background2, df.popDens2015[,c(1,3,4)], by="vbgem15",all.x=TRUE)
df.Background3$popDens15 <- df.Background3$z_bev15/df.Background3$fl15_sum
...
#saveRDS(object = ind.RR, file = "R/indRhineRuhr.rds")
...
sp.Background3 <- SpatialPoints(coords = df.Background3[,c("Longitude","Latitude")],
                               proj4string = CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs"))
sp.Background3 <- spTransform(sp.Background3,proj4string(raster.CLC12))
...
points(coordinates(spTransform(sp.Background3,CRS=proj4string(sLdf.roads.DE))),pch=16,cex=0.2)
plot(spTransform(gBuffer(sp.Background3[1,],width=5000),CRS=proj4string(sLdf.roads.DE)),add=TRUE,col="red")



spdf.Background3 <- SpatialPointsDataFrame(sp.Background3,data=df.Background3)
BufferSites <- gBuffer(spdf.Background3, byid = TRUE,
                       id = spdf.Background3$AirQualityStationEoICode,
                       width = radius)
...
df.Background3$Road14 <- 0
df.Background3$Road15 <- 0
df.Background3$Road16 <- 0
df.Background3$Road984 <- 0


for(i in 1:nrow(df.Background3)){
 ...
 if(nrow(sLdf.tmp)>0){
   ...
   if(nrow(sLdf.tmp2.14)>0){
     df.Background3$Road14[i] <- sum(SpatialLinesLengths(sLdf.tmp2.14))
   }
   if(nrow(sLdf.tmp2.15)>0){
     df.Background3$Road15[i] <- sum(SpatialLinesLengths(sLdf.tmp2.15))
   }
   if(nrow(sLdf.tmp2.16)>0){
       df.Background3$Road16[i] <- sum(SpatialLinesLengths(sLdf.tmp2.16))
   }
   if(nrow(sLdf.tmp2.984)>0){
       df.Background3$Road984[i] <- sum(SpatialLinesLengths(sLdf.tmp2.984))
   }
 }
}
...
par(mar=c(4,4,4,4))
plot(BufferSites[i,])
plot(spdf.Background3[i,],add=TRUE)
...
sLdf.roads.DE2 <- spTransform(sLdf.roads.DE, proj4string(spdf.Background3))
...
dat.ProximityToRoads <- data.frame(ID = df.Background3$AirQualityStationEoICode,
                                   Type = df.Background3$AirQualityStationType,
                                   Lon.WGS84 = df.Background3$Longitude,
                                   Lat.WGS84 = df.Background3$Latitude,
                                   DistPri = rep(NA, nrow(df.Background3)),
                                   DistSec = rep(NA, nrow(df.Background3)),
                                   DistMot = rep(NA, nrow(df.Background3)),
                                   DistLoc = rep(NA, nrow(df.Background3)))



for(i in 1:nrow(df.Background3)){
   dat.ProximityToRoads$DistPri[i] <- gDistance(spdf.Background3[i,], sLdf.roads.DE2.14)
   dat.ProximityToRoads$DistSec[i] <- gDistance(spdf.Background3[i,], sLdf.roads.DE2.15)
   dat.ProximityToRoads$DistMot[i] <- gDistance(spdf.Background3[i,], sLdf.roads.DE2.16)
   dat.ProximityToRoads$DistLoc[i] <- gDistance(spdf.Background3[i,], sLdf.roads.DE2.984)
}
...
# Choose i and check if for-loop does what it is supposed to do...
i <- 33
Buffer.tmp <- gBuffer(spdf.Background3[i, ], width = 7000)
...
names(df.Background3)
df.analysis <- df.Background3[,c(2,11,24,26:29,31:32,35,38:48,50:51,57:58,61:65)]
...
write.csv(df.analysis, file = "R/DATA/Data_built/DATA.csv")