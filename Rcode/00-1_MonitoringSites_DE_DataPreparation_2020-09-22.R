...
sPdf.boundaries.DE <- readOGR(dsn = "DataFull/Data_GADM", layer = "DEU_adm1")
...
admin.regions.2015 <- readOGR(dsn = "DataFull/Data_BKG/vg250-ew_ebenen",
                              ...
                              encoding = "UTF-8",
                              use_iconv = TRUE)
sum(admin.regions.2015$popDens == 0)			# 345 zero values!
...
names(df.analysis)[c(1:3,5:20,24,26:29)] <- c("AQeCode", "Y", "Year", "Lon", "Lat", "Alt",
